...
import { ChevronLeft, X, ChevronRight, Eye, Check, Ban } from "lucide-react";
...
const ManagerAppointmentChanges = () => {
    ...
    const handleReviewRequest = async (appointmentChangeId, isApproved) => {
        ...
        try {
            ...
            setError(`Không thể ${isApproved ? "chấp nhận" : "từ chối"} yêu cầu`);
        } finally {
    ...
    return (
        ...
        ${request.status === "PENDING" && !request.is_approved ? "bg-yellow-100 text-yellow-800"
                                                        : request.status === "PENDING" && request.is_approved ? "bg-green-100 text-green-800"
                                                            : request.status === "ACCEPTED" ? "bg-green-100 text-green-800"
                                                                : request.status === "REJECTED" ? "bg-red-100 text-red-800"
                                                                    : request.status === "CANCELLED" ? "bg-gray-100 text-gray-600"
                                                                        : "bg-gray-200 text-gray-800"
                                                    }`}>
                                                    {request.status === "PENDING" && !request.is_approved ? "Chờ xử lý"
                                                        : request.status === "PENDING" && request.is_approved ? "Đã duyệt"
        ...
        ${selectedRequest.status === "PENDING" && !selectedRequest.is_approved ? "bg-yellow-100 text-yellow-800"
                                                : selectedRequest.status === "PENDING" && selectedRequest.is_approved ? "bg-green-100 text-green-800"
                                                    : selectedRequest.status === "ACCEPTED" ? "bg-green-100 text-green-800"
                                                        : selectedRequest.status === "REJECTED" ? "bg-red-100 text-red-800"
                                                            : selectedRequest.status === "CANCELLED" ? "bg-gray-100 text-gray-600"
                                                                : "bg-gray-200 text-gray-800"}`}>
                                            {selectedRequest.status === "PENDING" && !selectedRequest.is_approved ? "Chờ xử lý"
                                                : selectedRequest.status === "PENDING" && selectedRequest.is_approved ? "Đã duyệt"
                                    ...
                                    {selectedRequest.status === "PENDING" && !selectedRequest.is_approved && (
                                        <div className="flex justify-end gap-2 pt-4">
                                            <Button
                                                variant="destructive"
                                                onClick={() => handleReviewRequest(selectedRequest.appointment_change_id, false)}
                                            ...
                                            <Button
                                                onClick={() => handleReviewRequest(selectedRequest.appointment_change_id, true)}
                                                disabled={isProcessing}
                                            >
...
export default ManagerAppointmentChanges;